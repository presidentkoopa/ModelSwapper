...
The weapon lying on the floor wears the same model you'll be holding once you pick it
up. Toggle: **Models on Floor Pickups**, or `rs_foreignmodels_pickups`.

The bind is the same two steps as the hand, aimed at the world sprite path instead of
the HUD one — `A_ChangeModel` on the item, then pin its sprite. No engine work was
needed: `hw_sprites.cpp` resolves world models through the same
`FindModelFrame(AActor*)` that prefers `modelData->modelDef` over the actor's class.

**A HUD block's numbers can't be reused.** Its `Scale` is tuned for a mesh at arm's
length in view space and its pose is whatever suited the hand — on the floor that reads
as a car-sized gun standing on its muzzle. So pickups get their own `MS_PU_*` blocks,
generated by `tools_gen_pickups.ps1`, which solves `Scale`, `Offset` and the three
rotation offsets from each mesh's own rest-frame vertices: principal axis by covariance
and power iteration, laid horizontal, then sized to the family's target length. The
mesh's own up is preserved rather than re-derived, so anything authored upright stays
upright and roll comes out near zero.

Two things the fit had to learn the hard way. **Collapsed surfaces are not geometry** —
both donor sets hide an inactive sub-mesh at rest by welding its vertices to a single
point, and VanAlek's revolver parks 257 of its 859 vertices on one spot 78 units off the
gun. Invisible in game, ruinous to a vertex fit. **A mesh with no dominant axis doesn't
get rotated at all** — fitting a barrel line to a grenade returns whichever diagonal the
vertices happened to favour.

Family targets come from the stock Doom pickup sprites these sit beside (SHOTA0 is 63px
wide, and a Doom sprite is one unit per pixel). Normalise per model, target per family:
the per-mesh measurement removes authoring variance, the per-family target is what keeps
a real size difference between a holdout and a crew-served weapon. `$OVERRIDE` in the
generator is where a mesh the rule gets wrong goes.

#### The disjoint-anchor invariant

HUD blocks anchor on hand sprites (`PISG`, `SHTG`, `CHGG`, …). Every pickup block anchors
on `SHOT A`, which appears in no HUD shelf row. **The two sprite spaces never overlap**,
so a binding that's stale in either direction resolves to *nothing* — `FindModelFrame`
returns null and the actor draws its sprite.

That is what lets the owner transition need no hook at all. In single player the floor
item and the carried weapon are the same actor (`Inventory::CreateCopy` returns `self`
when `GoAway()` is false), so the two binders share one `modelData` — but whichever is
wrong at any moment is inert. The hand binder re-points on selection, the pickup binder
on its next sweep, and in between you get a sprite.

Every failure mode degrades to "flat sprite". None degrades to a wrong model at a wrong
size.

#### Discovery

`WorldThingSpawned` binds immediately; a `ThinkerIterator` sweep once a second catches
what the event can't see, because `DropInventory` hands the *same* actor back to the map
with no spawn event. The sprite re-pin runs every tic against the tracked list only —
compare before writing, so a single-frame pickup costs one int compare, and a mod that
animates its pickup state doesn't flicker.

Savegames need nothing: `modelDef` serialises (`AActor::Serialize`), as does the pinned
sprite.

#### The model zoo

`ms_modelzoo` spawns one of every pickup model in a grid in front of you, in any map.
`map MSZOO` is the same grid in an empty room. It spawns the `MS_PU_*` display classes
directly, so what you're looking at is the MODELDEF block itself with no binder in
between — if a gun is wrong in the zoo, the generated numbers are wrong.

On the fork, every pickup block carries `PlacementCVars "ms_pu"`, so `ms_pu_scale`,
`ms_pu_pitch` and friends are live sliders that fold into the same transform the MODELDEF
values go through — a value found by eye transfers into the generator verbatim. They all
share one prefix on purpose: these are for finding a *global* correction, not for tuning
one gun. `build.ps1 -Static` strips the line, because an unrecognised MODELDEF property
is a fatal parse error on stock GZDoom rather than something it skips.
...
**Pickup models cover our own 44 donors only.** `tools_gen_pickups.ps1` measures the
meshes this pk3 ships. Loaded next to RS_Main, its `RS_GH_*`, `RS_PS_*` and `VR_*` rows
join the shelf and those have no measured pickup block, so a weapon wearing one of them
keeps its sprite on the floor while the model is still correct in your hands. Standalone
the coverage is complete, because the shelf drops those rows for want of their donor
classes. Not a failure mode — the disjoint anchors mean an unmapped donor resolves to
nothing and the item simply draws as its mod drew it.
...
zscript/
  ...
  RS_ForeignPickups.zs       floor-pickup binder, display class, model zoo
...
modeldef                     88 model blocks — 44 hud, 44 pickup
models/                      the meshes and skins
sprites/                     RSB0 — the ballistic round, and nothing else
maps/MSZOO.wad               the model zoo: one room, no editor numbers
tools_gen_bd21.ps1           generates the BD21 shelf from BD's own MODELDEFs
tools_gen_pickups.ps1        generates the pickup blocks from the meshes
